...
function hideButton1() {
    //When the sequence runs after the button has been pressed
    // this function will mostly work to remove
    //buttons. When the sequence has finished showing itself it
    //  will take in user inputs to check if they
    ...
    if(shownSequence === false){
        ...
        pointCheck(1);  //Depending on the square an input will be
        //  sent to the pointCheck function to see if the user
        //is correct. This is the same for all
        // hideButton functions
...
//  on the game when the sequence hasn't been played.
...
function showEndGameModal(){
    ...
    const endModal = new bootstrap.Modal(
        document.getElementById('endGameModalL2'));
    endModal.show();
}
...
function showButtonsPeriodically(){
    ...
    //loop felt too restrictive so making an iterator outside
    //  of the function made it less prone to
    //being reset on each recursive call or iteration.
    setTimeout(function() {
        i=i+1;
        //Eventually the condition of it being i<5 will be more automated
        // to allow for an increase of difficulty and more values coming up.
        if(i<maxInputs+1 && sequenceToMatch){
            ...
            //  only includes numbers 1-4 but array starts from (0-3).
            if(sequenceToMatch[i-1] === 1){
                showButton1();
                //Changed to be iniitialised each time to ensure
                //  there aren't repeat old timers being used.
        ...
        //There's a 2 second delay after each recursive
        //  call. It will change depending on difficulty.
    },2000);
}
...
function pointCheck(squareX){
    ...
    if(squareX === curSquare && curSquare){
        ...
        if(playerInputs === maxInputs){
            ...
            //This resets the values to allow for the user to
