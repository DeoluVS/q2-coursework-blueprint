...
//Since document.getElementById uses a string value to find
//  the element it is possible to use
//a for loop to iterate through all buttons but for now I will
//  keep it simple but implement it
...
function startMemorySequence(){
    document.getElementById("startBtnL3").disabled = true;
    sequenceToMatch = [];
...
function showEndGameModal(){
    document.getElementById("finalScoreL3").textContent = points;
    let percent = parseInt((points/highestScorePossible)*100);
    let percentScore = `${percent}%`;
    document.getElementById("finalPercentScoreL3").textContent =
    percentScore;
    const endModal = new bootstrap.Modal(
        document.getElementById("endGameModalL3"));
    endModal.show();
}

function restartGame(){
    const modalElement = document.getElementById("endGameModalL3");
...
function showButtonsPeriodically(){
    ...
    setTimeout(function() {
        ...
        if(i<maxInputs+1 && sequenceToMatch){
            //ChatGPT helped out for the check between the
            // current and previous element.
            //Its a quality of life improvement that allows for a value to
            //  appear on the square for any repeat
        ...
        //There"s a 1.5 second delay after each recursive call.
        //  It will change depending on difficulty.
    },1500);
}

function updateScoreBoard(){
    document.getElementById("scoreBoardL3").value = points;
}
...
//This is shown after the sequence has done being shown. It
//  checks if the user enters the right inputs in
//relation to the sequenceToMatch array.
function pointCheck(squareX){
    //Takes the first value at sequenceToMatch[0] removes it from
    //  the array and stores it in curSquare.
    ...
    if(squareX === curSquare && curSquare){
        ...
        sequencePoints+=100;
        updateScoreBoard();
        //playerInputs checks if the user has chosen the same amount
        // of inputs as the sequenceToMatch initial length (4),
        ...
        if(playerInputs === maxInputs){
            ...
            //This resets the values to allow for the user to play
            //  the next sequence
            correctInputs = 0;
            playerInputs = 0;
            sequencePoints = 0;
            //This is for testing and checks if all the values have
            //  been emptied out
            maxInputs+=1;
            startMemorySequence();
            resetButtons();
...
function changeBackgroundColor(levelOfAccuracy){
    ...
    //level 1 max points = (lengthOfSequence*100). And done
    //  by what amount of points obtained that level instead of
    //overall points. As that would make it really hard to ever
    //  get a full correct green win.
    //Green is maxpoint, orange is green/yellow is 80%,
    //  orange is 60%, orange/red is 40, and red is 20 and below.
